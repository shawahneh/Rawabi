...
public class JobsDB <B extends Job,T extends JobTable> extends DBHelper<Job> {

    private static JobsDB instance;
    public static JobsDB getInstance(Context context){
        if(instance == null){
            instance = new JobsDB(context);
        }
        return instance;
    }

    private JobsDB(Context context) {
    ...
    @Override
    protected void fillBeanFromCursor(Cursor rs, Job bean) {
